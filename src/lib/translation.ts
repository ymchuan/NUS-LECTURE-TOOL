...
export async function translateArchivedSegment(segment: TranscriptSegment, segments: TranscriptSegment[], settings: CourseSettings, preferences: SummaryPreferences, onUpdate: (segment: TranscriptSegment) => void): Promise<TranscriptSegment> {
  ...
  try {
    ...
    await Promise.race([
      Promise.all([stream, invoke("translate_segment", { request: { segmentId: requestId, english: segment.english, previousEnglish: previous?.english ?? null, courseName: settings.courseName, glossary: settings.glossary, provider: preferences.translationProvider, workspaceId: preferences.alibabaWorkspaceId, model: preferences.translationProvider === "local" ? preferences.localTranslationModel : preferences.translationModel, localEndpoint: preferences.localTranslationEndpoint } })]),
