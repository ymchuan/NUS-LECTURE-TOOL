...
import { TranslationDialog, type RetryPreferences } from "./components/TranslationDialog";
...
import { translateTranscriptSegment } from "./lib/translation";
import type {
  Course,
  ChatAnswer,
  ChatCitation,
...
const defaultSummaryPreferences: SummaryPreferences = {
  ...
  transcriptionModel: DEEPGRAM_NOVA_MODEL,
  microphoneDeviceId: "",
  textProvider: "openai",
  alibabaWorkspaceId: "",
...
function loadSummaryPreferences(): SummaryPreferences {
  try {
    ...
    const preferences = {
      ...
      ...saved,
      // Keep old profiles (and malformed/null values) usable after introducing
      // the custom OpenAI-compatible endpoint setting.
      openaiBaseUrl: typeof saved?.openaiBaseUrl === "string" && saved.openaiBaseUrl.trim()
        ? saved.openaiBaseUrl
        : defaultSummaryPreferences.openaiBaseUrl,
      ollamaBaseUrl: typeof saved?.ollamaBaseUrl === "string" && saved.ollamaBaseUrl.trim()
        ? saved.ollamaBaseUrl
        : defaultSummaryPreferences.ollamaBaseUrl,
      ollamaTranslationModel: typeof saved?.ollamaTranslationModel === "string" && saved.ollamaTranslationModel.trim()
        ? saved.ollamaTranslationModel
        : defaultSummaryPreferences.ollamaTranslationModel,
      ollamaCloudTranslationModel: typeof saved?.ollamaCloudTranslationModel === "string"
        ? saved.ollamaCloudTranslationModel
        : "",
    };
...
function CourseDialog({
  ...
  hasOpenAiApiKey,
  hasOpenAiTranslationApiKey,
  hasOpenAiSummaryApiKey,
  hasOpenAiLectureApiKey,
  hasAlibabaApiKey,
  hasAlibabaAsrApiKey,
  hasDeepgramApiKey,
  courseLocked,
  ...
  hasOpenAiTranslationApiKey: boolean;
  hasOpenAiSummaryApiKey: boolean;
  hasOpenAiLectureApiKey: boolean;
  ...
  onSave: (
    ...
    openAiLectureApiKey: string,
    alibabaApiKey: string,
    alibabaAsrApiKey: string,
    deepgramApiKey: string,
    summaryPreferences: SummaryPreferences,
  ) => Promise<void>;
  onDeleteKey: (provider: "openai" | "openai-translation" | "openai-summary" | "openai-lecture" | "alibaba" | "alibaba-asr" | "deepgram") => Promise<void>;
}) {
  const [draft, setDraft] = useState<CourseDraft>({ code: "", name: "", description: "" });
  const [terms, setTerms] = useState<GlossaryTerm[]>([]);
  const [openAiApiKey, setOpenAiApiKey] = useState("");
  const [openAiTranslationApiKey, setOpenAiTranslationApiKey] = useState("");
  const [openAiSummaryApiKey, setOpenAiSummaryApiKey] = useState("");
  const [openAiLectureApiKey, setOpenAiLectureApiKey] = useState("");
  ...
  useEffect(() => {
    ...
    setOpenAiApiKey("");
    setOpenAiTranslationApiKey("");
    setOpenAiSummaryApiKey("");
    setOpenAiLectureApiKey("");
    setAlibabaApiKey("");
    setAlibabaAsrApiKey("");
    setDeepgramApiKey("");
        ...
        <form className="course-editor" onSubmit={submit}>
          ...
          <div className="course-editor-main">
            ...
            <div className="form-section summary-preferences">
              ...
              <div className="model-provider-control" aria-label="实时转写供应商">
                <button
                ...
                <div className="form-section api-section">
                  <div className="label-row">
                    ...
                    <span className={`key-state ${hasDeepgramApiKey ? "saved" : "missing"}`}>
                      {hasDeepgramApiKey ? "已保存至系统凭据库" : "未保存"}
                    </span>
                  </div>
                  <div className="key-input-row">
                    ...
                    />
                    {hasDeepgramApiKey && (
                      <button className="icon-button danger-quiet" type="button" title="删除 Deepgram API Key" onClick={() => void onDeleteKey("deepgram")}>
                        <Trash2 size={18} />
                      </button>
                    )}
                  </div>
            ...
            <div className="form-section summary-preferences">
              <div className="section-title-row">
                <div>
                  <strong>翻译与总结模型</strong>
                  <span>{summaryPreferences.summaryModel}</span>
                </div>
              </div>
              <div className="model-provider-control" aria-label="文本模型供应商">
                <button
              ...
              <label className="model-select-row" htmlFor="translation-model">
                ...
                <datalist id="translation-model-options">
                  {summaryPreferences.textProvider === "openai" ? (
                    <>
                      <option value="gpt-5.6-luna">GPT-5.6 Luna · 低延迟省成本</option>
                      <option value="gpt-5.6-terra">GPT-5.6 Terra · 均衡</option>
                      <option value="gpt-5.6-sol">GPT-5.6 Sol · 高质量</option>
                      <option value="gpt-5.4">GPT-5.4 · 稳定</option>
                    </>
                  ) : (
                    <>
              ...
              <label className="model-select-row" htmlFor="summary-model">
                ...
                <datalist id="summary-model-options">
                  {summaryPreferences.textProvider === "openai" ? (
                    <>
                      <option value="gpt-5.6-luna">GPT-5.6 Luna · 经济</option>
                      <option value="gpt-5.6-terra">GPT-5.6 Terra · 推荐</option>
                      <option value="gpt-5.6-sol">GPT-5.6 Sol · 质量优先</option>
                      <option value="gpt-5.4">GPT-5.4 · 稳定</option>
                    </>
                  ) : (
                    <>
              ...
              <label className="model-select-row" htmlFor="lecture-summary-model">
                ...
                <input
                  ...
                  list="lecture-summary-model-options"
                  required maxLength={100} spellCheck={false}
                ...
                />
                <datalist id="lecture-summary-model-options">
                  {summaryPreferences.textProvider === "openai" ? (
                    <>
                      <option value="gpt-5.6-luna">GPT-5.6 Luna · 经济</option>
                      <option value="gpt-5.6-terra">GPT-5.6 Terra · 均衡</option>
              ...
              <label className="setting-toggle">
                ...
                <input
                  type="checkbox"
                  checked={summaryPreferences.textProvider !== "ollama" && summaryPreferences.webSearchEnabled}
                  disabled={summaryPreferences.textProvider === "ollama"}
                  onChange={(event) =>
                    setSummaryPreferences({
                      ...summaryPreferences,
            ...
            {(summaryPreferences.transcriptionProvider === "openai"
              ...
              <label className="model-select-row openai-base-url-row" htmlFor="openai-base-url">
                <span>
                  <strong>OpenAI Base URL</strong>
                  <small>第三方 OpenAI 兼容平台填写完整地址，例如 https://example.com/v1</small>
                </span>
                <input
                  id="openai-base-url"
                  type="url"
                  autoComplete="off"
                  spellCheck={false}
                  value={summaryPreferences.openaiBaseUrl}
              ...
              <p className="openai-base-url-note">
                自定义 Base URL 仅用于翻译、总结和问答等文本接口；OpenAI 实时语音转写仍使用官方 Realtime 服务。
              </p>
              <div className="openai-key-slots">
                <p className="field-help">可为不同功能配置独立的 OpenAI Key；未填写时会回退到上面的通用 Key。</p>
                {([
                  ["translation", "实时翻译 Key", openAiTranslationApiKey, setOpenAiTranslationApiKey, hasOpenAiTranslationApiKey],
                  ["summary", "阶段总结 Key", openAiSummaryApiKey, setOpenAiSummaryApiKey, hasOpenAiSummaryApiKey],
                  ["lecture", "整课总结 / 问答 Key", openAiLectureApiKey, setOpenAiLectureApiKey, hasOpenAiLectureApiKey],
                ] as const).map(([slot, label, value, setter, saved]) => (
                  <div className="api-slot-row" key={slot}>
                    <div className="label-row">
                      <label htmlFor={`openai-${slot}-api-key`}>{label}</label>
                      <span className={`key-state ${saved ? "saved" : "missing"}`}>
                        {saved ? "已保存" : "未保存"}
                      </span>
                    </div>
                    <div className="key-input-row">
                      <input
                        id={`openai-${slot}-api-key`}
                        type="password"
                        autoComplete="off"
                        value={value}
                        placeholder={saved ? "输入新密钥可替换" : "可选，留空则使用通用 Key"}
                      ...
                      {saved && (
                        <button
                          ...
                          type="button"
                          title={`删除${label}`}
                          onClick={() => void onDeleteKey(`openai-${slot}` as "openai-translation" | "openai-summary" | "openai-lecture")}
                        >
                          <Trash2 size={18} />
                        </button>
                      )}
            ...
            {(summaryPreferences.textProvider === "alibaba"
              ...
              <div className="form-section api-section">
                ...
                <input
                  ...
                  pattern="llm-[a-z0-9-]+"
                  maxLength={80}
                  title="请输入类似 llm-xxxxxxxx 的业务空间 ID"
    ...
    try {
      await invoke<ChatAnswer>("ask_lecture_chat", {
        request: {
          ...
          openaiKeySlot: "lecture",
          model: preferences.lectureSummaryModel,
          webSearch: preferences.textProvider !== "ollama" && webSearch,
        },
      });
    ...
    <div className="chat-panel">
      <div className="chat-controls">
        ...
        <label className={`chat-web-toggle${asking ? " disabled" : ""}`}>
          <input type="checkbox" checked={preferences.textProvider !== "ollama" && webSearch} disabled={asking || preferences.textProvider === "ollama"} onChange={(event) => setWebSearch(event.target.checked)} />
          <Globe2 size={14} />
          联网核验
        </label>
      </div>
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (slides) {
        options.slides = {
          name: slides.name,
          mimeType: slides.type || (slides.name.toLowerCase().endsWith(".pdf")
            ? "application/pdf"
      ...
      <section className="settings-dialog lecture-setup-dialog" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <form className="lecture-setup-form" onSubmit={submit}>
          ...
          <label className={`slides-dropzone ${slides ? "has-file" : ""}`}>
            ...
            <span>{slides ? `${(slides.size / 1024 / 1024).toFixed(1)} MB` : "PDF 或 PPTX，可暂不上传"}</span>
          </label>
...
function EndLectureDialog({
  ...
  open: boolean;
  onClose: () => void;
  onFinish: (generateSummary: boolean) => void;
}) {
  if (!open) return null;
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="settings-dialog end-lecture-dialog" role="dialog" aria-modal="true" aria-labelledby="end-lecture-title" onMouseDown={(event) => event.stopPropagation()}>
        <header className="dialog-header">
          <div><p className="eyebrow">结束课堂</p><h2 id="end-lecture-title">要生成整课总结吗？</h2></div>
          <button className="icon-button" type="button" title="取消" onClick={onClose}><X size={19} /></button>
        </header>
        <p className="dialog-description">整课总结会读取本节课的全部转写并调用文本模型。选择“不生成”可以立即保存课堂，稍后也能从历史记录中重新生成。</p>
        <div className="dialog-actions end-lecture-actions">
          <button className="secondary-button" type="button" onClick={() => onFinish(false)}>结束并保存，不生成</button>
          <button className="primary-button" type="button" onClick={() => onFinish(true)}><Sparkles size={15} />结束并生成总结</button>
        </div>
      </section>
    </div>
  );
    ...
    >
      ...
      <div className="transcript-copy">
        <div className="transcript-line-header">
          ...
          {onRetryTranslation && segment.state !== "interim" && (
            <div className="transcript-translation-actions">
            <button
              className="icon-button retry-translation-button"
              type="button"
              title="翻译此语段"
              aria-label="翻译此语段"
              disabled={segment.state === "translating"}
              onClick={() => onRetryTranslation(segment.id)}
            >
              {segment.state === "translating" ? <LoaderCircle size={14} className="spinning" /> : <Languages size={14} />}
            </button>
            </div>
          )}
        </div>
        ...
        {segment.state === "error" && <small className="translation-error">翻译未完成</small>}
      </div>
...
export default function App() {
  ...
  const [hasOpenAiTranslationApiKey, setHasOpenAiTranslationApiKey] = useState(false);
  const [hasOpenAiSummaryApiKey, setHasOpenAiSummaryApiKey] = useState(false);
  const [hasOpenAiLectureApiKey, setHasOpenAiLectureApiKey] = useState(false);
  ...
  const [translationRequest, setTranslationRequest] = useState<{ segmentId: string; lectureId: number | null } | null>(null);
  ...
  const [endLectureDialogOpen, setEndLectureDialogOpen] = useState(false);
  ...
  const archivedTranslationsRef = useRef(new Set<string>());
  ...
  const translationSegment = translationRequest?.lectureId === displayLectureId
    ? displaySegments.find((segment) => segment.id === translationRequest.segmentId) : undefined;
  useEffect(() => { setTranslationRequest(null); }, [displayLectureId]);
  const saveRetryPreferences = (patch: RetryPreferences) => {
    const next = { ...summaryPreferences, ...patch };
    localStorage.setItem("lecture-assistant-summary-preferences", JSON.stringify(next));
    setSummaryPreferences(next);
  };
  const finishLecture = useCallback((generateSummary: boolean) => {
    setEndLectureDialogOpen(false);
    void session.stop(generateSummary);
  }, [session]);
    ...
    const bootstrap = async () => {
      try {
        void Promise.all([
          ...
          invoke<boolean>("has_provider_api_key", { provider: "openai-translation" }),
          invoke<boolean>("has_provider_api_key", { provider: "openai-summary" }),
          invoke<boolean>("has_provider_api_key", { provider: "openai-lecture" }),
          ...
          setHasOpenAiTranslationApiKey(openAiTranslationSaved);
          setHasOpenAiSummaryApiKey(openAiSummarySaved);
          setHasOpenAiLectureApiKey(openAiLectureSaved);
  ...
  const regenerateArchivedLectureSummary = async () => {
    if (!isTauri() || !archivedLecture || regeneratingLectureSummary) return;
    const candidates = archivedLecture.segments.filter(
      (segment) => segment.state !== "interim" && segment.english.trim(),
    );
    if (!candidates.length) {
      setAppError("这节历史课堂没有可用于重新总结的稳定转写");
      return;
    }

    setRegeneratingLectureSummary(true);
    try {
      const existingLectureSummary = archivedLecture.summaries.find(
        (summary) => summary.kind === "lecture",
      );
      const topicSummaries = archivedLecture.summaries.filter(
        (summary) => summary.kind !== "lecture",
      );
      const summary = await invoke<TopicSummary>("generate_topic_summary", {
        request: {
          ...
          ollamaBaseUrl: summaryPreferences.ollamaBaseUrl,
          openaiKeySlot: "lecture",
      ...
      await invoke("save_lecture_snapshot", {
        snapshot: {
          lectureId: archivedLecture.lectureId,
          elapsedMs: archivedLecture.elapsedMs,
          status: archivedLecture.status,
          segments: archivedLecture.segments,
          summaries,
        },
      });
      setArchivedLecture((current) => current ? { ...current, summaries } : current);
  ...
  const retryArchivedTranslation = async (segmentId: string, target: TranslationTarget = "configured") => {
    const lecture = archivedLecture;
    const segment = lecture?.segments.find((item) => item.id === segmentId);
    if (!isTauri() || !lecture || !segment || segment.state === "interim") return;
    const taskKey = `${lecture.lectureId}:${segmentId}`;
    if (archivedTranslationsRef.current.has(taskKey)) return;
    archivedTranslationsRef.current.add(taskKey);
    let translated = segment;
    try {
      await translateTranscriptSegment({
        segment, segments: lecture.segments, settings, preferences: summaryPreferences, target,
        onUpdate: (next) => {
          translated = next;
          setArchivedLecture((current) => current?.lectureId === lecture.lectureId
            ? { ...current, segments: current.segments.map((item) => item.id === segmentId ? next : item) }
            : current);
        },
      });
    } catch (reason) {
      setAppError(`补翻失败：${String(reason)}`);
    } finally {
      try {
        await invoke("save_transcript_translation", {
          lectureId: lecture.lectureId, segmentId,
          chinese: translated.chinese, state: translated.state,
        });
      } catch (reason) {
        setAppError(`补翻结果未保存：${String(reason)}`);
      }
      archivedTranslationsRef.current.delete(taskKey);
    }
  };

  const openArchivedLecture = async (lecture: LectureListItem) => {
    if (!isTauri()) return;
    if (lecture.id === session.lectureId) {
      await returnToCurrentLecture();
  ...
  const saveCourse = async (
    ...
    if (!isTauri()) {
      ...
      if (openAiApiKey.trim() || openAiTranslationApiKey.trim() || openAiSummaryApiKey.trim() || openAiLectureApiKey.trim() || alibabaApiKey.trim() || alibabaAsrApiKey.trim() || deepgramApiKey.trim()) {
    ...
    const courseRows = await invoke<Course[]>("list_courses");
    setCourses(courseRows);
    if (saved.id === settings.courseId) {
      setTerms(savedTerms);
      setSettings(settingsForCourse(saved, savedTerms));
    } else {
    ...
    if (openAiTranslationApiKey.trim()) {
      await invoke("save_provider_api_key", { provider: "openai-translation", apiKey: openAiTranslationApiKey });
      setHasOpenAiTranslationApiKey(true);
    }
    if (openAiSummaryApiKey.trim()) {
      await invoke("save_provider_api_key", { provider: "openai-summary", apiKey: openAiSummaryApiKey });
      setHasOpenAiSummaryApiKey(true);
    }
    if (openAiLectureApiKey.trim()) {
      await invoke("save_provider_api_key", { provider: "openai-lecture", apiKey: openAiLectureApiKey });
      setHasOpenAiLectureApiKey(true);
    }
  ...
  const deleteApiKey = async (provider: "openai" | "openai-translation" | "openai-summary" | "openai-lecture" | "alibaba" | "alibaba-asr" | "deepgram") => {
    if (!isTauri()) return;
    await invoke("delete_provider_api_key", { provider });
    if (provider === "openai") setHasOpenAiApiKey(false);
    else if (provider === "openai-translation") setHasOpenAiTranslationApiKey(false);
    else if (provider === "openai-summary") setHasOpenAiSummaryApiKey(false);
    else if (provider === "openai-lecture") setHasOpenAiLectureApiKey(false);
    else if (provider === "alibaba") setHasAlibabaApiKey(false);
    else if (provider === "alibaba-asr") setHasAlibabaAsrApiKey(false);
    else setHasDeepgramApiKey(false);
  };
  ...
  const beginLecture = () => {
    ...
    const missingTextKey = summaryPreferences.textProvider === "openai"
      ? (!hasOpenAiApiKey && !hasOpenAiTranslationApiKey)
        || (!hasOpenAiApiKey && !hasOpenAiSummaryApiKey)
      : summaryPreferences.textProvider === "alibaba" ? !hasAlibabaApiKey : false;
    if (missingAsrKey || missingTextKey) {
      setSettingsOpen(true);
      return;
    }
  ...
  return (
    <div className="app-shell">
      ...
      <main className="workspace">
        <section className="summary-pane" aria-labelledby="summary-heading">
          <header className="pane-header">
            ...
            {leftPaneTab === "summary" && <span className="count-badge">{displaySummaries.length}</span>}
          </header>
            ...
            {displaySegments.length ? (
              displaySegments.map((segment) => (
                <TranscriptRow
                  ...
                  onRetryTranslation={isTauri() && !session.isEnding && session.status !== "demo"
                    ? (segmentId) => setTranslationRequest({ segmentId, lectureId: displayLectureId }) : undefined}
                />
              ))
            ) : (
          ...
          {!isActive ? (
            ...
            <>
              ...
              <button
                ...
                onClick={() => setEndLectureDialogOpen(true)}
              >
      ...
      <CourseDialog
        ...
        hasOpenAiTranslationApiKey={hasOpenAiTranslationApiKey}
        hasOpenAiSummaryApiKey={hasOpenAiSummaryApiKey}
        hasOpenAiLectureApiKey={hasOpenAiLectureApiKey}
      ...
      {translationSegment && !session.isEnding && <TranslationDialog
        key={`${translationRequest?.lectureId}:${translationSegment.id}`}
        segment={translationSegment}
        preferences={summaryPreferences}
        onSave={saveRetryPreferences}
        onClose={() => setTranslationRequest(null)}
        onTranslate={(target) => {
          setTranslationRequest(null);
          void (isViewingHistory ? retryArchivedTranslation : session.retryTranslation)(translationSegment.id, target);
        }}
      />}

      <EndLectureDialog
        open={endLectureDialogOpen}
        onClose={() => setEndLectureDialogOpen(false)}
