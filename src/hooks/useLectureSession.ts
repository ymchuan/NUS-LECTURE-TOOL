...
export function useLectureSession(
  ...
  const endedLectureIdRef = useRef<number | null>(null);
  ...
  const alibabaAsrActiveRef = useRef(false);
  const deepgramAsrActiveRef = useRef(false);
  ...
  const translatingSegmentsRef = useRef(new Set<string>());
  ...
  const startAudioMeter = useCallback((stream: MediaStream, forwardProvider: "alibaba" | "deepgram" | null = null) => {
    ...
    if (forwardProvider) {
      ...
      processor.onaudioprocess = (event) => {
        if (forwardProvider === "alibaba" && !alibabaAsrActiveRef.current) return;
        if (forwardProvider === "deepgram" && !deepgramAsrActiveRef.current) return;
        ...
        asrAudioQueueRef.current = asrAudioQueueRef.current
          .then(() => invoke<void>(forwardProvider === "alibaba" ? "send_alibaba_asr_audio" : "send_deepgram_asr_audio", {
            audioBase64: pcmToBase64(pcm),
          }))
          .catch((reason) => {
            if (forwardProvider === "alibaba" && !alibabaAsrActiveRef.current) return;
            if (forwardProvider === "deepgram" && !deepgramAsrActiveRef.current) return;
            setError(`${forwardProvider === "deepgram" ? "Deepgram" : "阿里云"}实时转写音频发送失败：${String(reason)}`);
            updateStatus("error");
          });
      };
  ...
  const startAlibabaAsr = useCallback(async (extraKeywords: string[] = []) => {
    asrAudioQueueRef.current = Promise.resolve();
    await invoke("start_alibaba_asr", {
      request: {
        model: summaryPreferences.transcriptionModel,
    ...
    alibabaAsrActiveRef.current = true;
  }, [settings.hotwords, summaryPreferences.alibabaWorkspaceId, summaryPreferences.transcriptionModel]);

  const finishAlibabaAsr = useCallback(async () => {
    if (!alibabaAsrActiveRef.current) return;
    alibabaAsrActiveRef.current = false;
    await asrAudioQueueRef.current.catch(() => undefined);
    await invoke("finish_alibaba_asr");
  }, []);

  const startDeepgramAsr = useCallback(async () => {
    asrAudioQueueRef.current = Promise.resolve();
    await invoke("start_deepgram_asr", {
      request: {
        model: summaryPreferences.transcriptionModel,
        elapsedOffsetMs: elapsedMsRef.current,
      },
    });
    deepgramAsrActiveRef.current = true;
  }, [summaryPreferences.transcriptionModel]);

  const finishDeepgramAsr = useCallback(async () => {
    if (!deepgramAsrActiveRef.current) return;
    deepgramAsrActiveRef.current = false;
    await asrAudioQueueRef.current.catch(() => undefined);
    await invoke("finish_deepgram_asr");
  }, []);

  const requestTranslation = useCallback(
    async (segmentId: string, english: string, target: TranslationTarget = "configured") => {
      const lectureId = lectureIdRef.current ?? endedLectureIdRef.current;
      const taskKey = `${lectureId}:${segmentId}`;
      if (translatingSegmentsRef.current.has(taskKey)) return;
      translatingSegmentsRef.current.add(taskKey);
      const segment = segmentsRef.current.find((item) => item.id === segmentId) ?? {
        id: segmentId, itemId: segmentId, startMs: elapsedMsRef.current,
        english, chinese: "", state: "translating" as const,
      };
      try {
        const translated = await translateTranscriptSegment({
          segment: { ...segment, english }, segments: segmentsRef.current,
          settings, preferences: summaryPreferences, target,
          onUpdate: (next) => {
            if ((lectureIdRef.current ?? endedLectureIdRef.current) !== lectureId) return;
            updateSegments((current) => current.map((item) => item.id === segmentId ? next : item));
          },
        });
        if (lectureId !== null && endedLectureIdRef.current === lectureId) {
          await invoke("save_transcript_translation", {
            lectureId, segmentId, chinese: translated.chinese, state: translated.state,
          });
        }
      } catch (reason) {
        if ((lectureIdRef.current ?? endedLectureIdRef.current) === lectureId) setError(`翻译失败：${String(reason)}`);
      } finally {
        translatingSegmentsRef.current.delete(taskKey);
      }
    },
    [settings, summaryPreferences, updateSegments],
  );

  const queueTranslation = useCallback((segmentId: string, english: string, target: TranslationTarget = "configured") => {
    const task = requestTranslation(segmentId, english, target);
    pendingTranslationsRef.current.add(task);
    void task.finally(() => pendingTranslationsRef.current.delete(task));
    return task;
  }, [requestTranslation]);

  const retryTranslation = useCallback(async (segmentId: string, target: TranslationTarget = "configured") => {
    const segment = segmentsRef.current.find((item) => item.id === segmentId);
    if (!isTauri() || endingRef.current || !segment || segment.state === "interim" || !segment.english.trim()) return;
    await queueTranslation(segmentId, segment.english, target);
  }, [queueTranslation]);
  ...
  useEffect(() => {
    ...
    const asrListener = listen<AlibabaAsrEvent>("alibaba-asr-event", ({ payload }) => {
      ...
      if (payload.kind === "delta") {
        ...
        setError(payload.text || "阿里实时转写出现错误");
        updateStatus("error");
      }
    });
    const deepgramListener = listen<AlibabaAsrEvent>("deepgram-asr-event", ({ payload }) => {
      if (payload.kind === "delta") {
        updateSegments((current) => discardSupersededInterims(upsertTranscriptText(current, payload.itemId, payload.text, payload.startMs), payload.itemId));
      } else if (payload.kind === "final") {
        const transcript = payload.text.trim();
        if (!transcript) return;
        updateSegments((current) => finalizeTranscript(current, payload.itemId, transcript, payload.startMs));
        queueTranslation(payload.itemId, transcript);
      } else {
        setError(payload.text || "Deepgram 实时转写出现错误");
        updateStatus("error");
      }
    });

    return () => {
      void asrListener.then((unlisten) => unlisten());
      void deepgramListener.then((unlisten) => unlisten());
    };
  }, [queueTranslation, updateSegments, updateStatus]);
  ...
  const cleanupMedia = useCallback(() => {
    ...
    if (alibabaAsrActiveRef.current) {
      alibabaAsrActiveRef.current = false;
      void asrAudioQueueRef.current
        .catch(() => undefined)
        .then(() => invoke("finish_alibaba_asr"));
    }
    if (deepgramAsrActiveRef.current) {
      deepgramAsrActiveRef.current = false;
      void asrAudioQueueRef.current
        .catch(() => undefined)
        .then(() => invoke("finish_deepgram_asr"));
    }
  ...
  const startLive = useCallback(async (options?: StartLectureOptions) => {
    ...
    if (!isResuming) {
      endedLectureIdRef.current = null;
      setSegments([]);
      segmentsRef.current = [];
      setSummaries([]);
    ...
    try {
      ...
      if (summaryPreferences.transcriptionProvider === "alibaba") {
        await startAlibabaAsr(documentKeywords);
        startAudioMeter(stream, "alibaba");
        updateStatus("live");
        startTimer();
        return lectureIdRef.current;
      }
      if (summaryPreferences.transcriptionProvider === "deepgram") {
        await startDeepgramAsr();
        startAudioMeter(stream, "deepgram");
        updateStatus("live");
        startTimer();
        return lectureIdRef.current;
      }
  ...
  }, [cleanupMedia, handleRealtimeEvent, persistSnapshot, settings, startAlibabaAsr, startAudioMeter, startDeepgramAsr, startTimer, summaryPreferences, updateStatus]);
  ...
  const togglePause = useCallback(async () => {
    if (!streamRef.current || !["live", "paused"].includes(statusRef.current)) return;
    ...
    if (shouldPause) {
      ...
      if (summaryPreferences.transcriptionProvider === "alibaba") {
        await finishAlibabaAsr().catch((reason) => setError(String(reason)));
      }
      if (summaryPreferences.transcriptionProvider === "deepgram") {
        await finishDeepgramAsr().catch((reason) => setError(String(reason)));
      }
      stopTimer();
      updateStatus("paused");
      ...
      try {
        if (summaryPreferences.transcriptionProvider === "alibaba") {
          await startAlibabaAsr(documentKeywordsRef.current);
        }
        if (summaryPreferences.transcriptionProvider === "deepgram") {
          await startDeepgramAsr();
        }
  ...
  }, [finishAlibabaAsr, finishDeepgramAsr, persistSnapshot, startAlibabaAsr, startDeepgramAsr, startTimer, stopTimer, summaryPreferences.transcriptionProvider, updateStatus]);
  ...
  const generateSummary = useCallback(
    async (automatic = false, kind: "topic" | "lecture" = "topic") => {
      ...
      setSummaryStatus(summaryPreferences.webSearchEnabled && summaryPreferences.textProvider !== "ollama" ? "searching" : "generating");
      ...
      try {
        const summary = await invoke<TopicSummary>("generate_topic_summary", {
          request: {
            ...
            openaiKeySlot: kind === "lecture" ? "lecture" : "summary",
            model: summaryModelForKind(summaryPreferences, kind),
            webSearch: summaryPreferences.webSearchEnabled && summaryPreferences.textProvider !== "ollama",
          },
        });
        updateSummaries((current) => {
          const withoutExistingLecture = kind === "lecture"
  ...
  const stop = useCallback(async (generateLectureSummary = true) => {
    ...
    try {
      ...
      if (generateLectureSummary
        && segmentsRef.current.some((segment) => segment.state !== "interim" && segment.english.trim())) {
      ...
      endedLectureIdRef.current = lectureId;
      lectureIdRef.current = null;
      pendingTranslationsRef.current.clear();
      setLectureId(null);
      updateStatus("idle");
  ...
  }, [cleanupMedia, finishAlibabaAsr, finishDeepgramAsr, generateSummary, persistSnapshot, summaryPreferences.transcriptionProvider, updateStatus, waitForPendingTranslations]);
