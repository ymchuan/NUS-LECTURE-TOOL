...
const LegacyPowerPointPreview = lazy(() => import("./LegacyPowerPointPreview"));
    ...
    const load = async () => {
      ...
      try {
        ...
        loadingTask = pdfjs.getDocument({
          data: base64ToBytes(source.dataBase64),
  ...
  useEffect(() => {
    ...
    const render = async () => {
      ...
      try {
        ...
        await previewer.load(bytesToArrayBuffer(base64ToBytes(source.dataBase64)));
        if (cancelled) {
          previewer.destroy();
          return;
        }
...
export function DocumentPagePreview(props: PreviewProps) {
  const lowerName = props.source.name.toLowerCase();
  const isPdf = props.source.mimeType === "application/pdf"
    || lowerName.endsWith(".pdf");
  if (isPdf) return <PdfPagePreview {...props} />;
  const isLegacyPpt = props.source.mimeType === "application/vnd.ms-powerpoint"
    || lowerName.endsWith(".ppt");
  if (isLegacyPpt) {
    return (
      <Suspense fallback={<PreviewStatus label="正在加载旧版 PowerPoint 解析器" />}>
        <LegacyPowerPointPreview {...props} />
      </Suspense>
    );
  }
  return <PptxPagePreview {...props} />;
}
