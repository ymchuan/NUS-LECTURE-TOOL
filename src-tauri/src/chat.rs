...
fn ollama_chat_payload(model: &str, instructions: &str, input: &str) -> Value {
    json!({
        "model": model,
        "messages": [
            {
                "role": "system",
                "content": format!("{instructions}\n当前是离线本地模型，没有联网检索工具。不得声称已联网、检索或核验网络来源；没有课堂资料支持的内容必须标注为模型通用知识。")
            },
        ...
        "options": {
            "temperature": 0.2,
            "num_predict": 2200,
            "num_ctx": crate::OLLAMA_CONTEXT_TOKENS
        }
    })
}
...
pub async fn ask_lecture_chat(
    ...
    let base_url = crate::text_provider_base_url(
        &request.provider,
    ...
    let mut payload = if request.provider == "ollama" {
        crate::validate_ollama_input(&[instructions, &input])?;
        ollama_chat_payload(&request.model, instructions, &input)
    } else {
        json!({
            "model": request.model,
    ...
    if request.web_search && request.provider != "ollama" {
    ...
    let api_key = if request.provider == "ollama" {
        ...
        let slot = match request.openai_key_slot.as_str() {
            "translation" => "openai-translation",
            "summary" => "openai-summary",
            "lecture" => "openai-lecture",
            _ => "openai-lecture",
        };
        crate::read_provider_api_key(slot)?
    } else {
        crate::read_provider_api_key(&request.provider)?
    };
    ...
    if request.provider == "ollama" {
        crate::validate_ollama_response(&response)?;
    }
    let answer = output_text(&response).ok_or_else(|| "模型没有返回回答".to_string())?;
    let web_citations = if request.provider == "ollama" {
        Vec::new()
    } else {
        web_sources(&response)
    };
    let (answer, citations) = verified_answer_citations(answer, allowed_citations, web_citations);
    let (thread, message) = {
...
mod tests {
    ...
    #[test]
    fn builds_native_ollama_chat_payload_without_tools() {
        let payload = ollama_chat_payload("qwen2.5:7b", "你是助理。", "问题");
        assert_eq!(payload["model"], "qwen2.5:7b");
        assert_eq!(payload["stream"], false);
        assert_eq!(payload["think"], false);
        assert_eq!(payload["keep_alive"], "5m");
        assert_eq!(payload["options"]["num_ctx"], crate::OLLAMA_CONTEXT_TOKENS);
    ...
    #[test]
    fn extracts_native_ollama_answer() {
        let response = json!({"message": {"content": "本地回答"}});
        assert_eq!(output_text(&response).as_deref(), Some("本地回答"));
    }
}
