...
pub struct SummaryRequest {
    ...
    /// Optional OpenAI-compatible API base URL; empty uses the official API.
    #[serde(default)]
    pub openai_base_url: String,
    #[serde(default)]
    pub ollama_base_url: String,
    #[serde(default)]
    pub openai_key_slot: String,
    pub model: String,
    pub web_search: bool,
}
...
fn compact_summary_retry_payload(payload: &Value, is_lecture: bool) -> Value {
    ...
    let max_output_tokens = if is_lecture { 22_000 } else { 7_000 };
    if let Some(messages) = retry.get_mut("messages").and_then(Value::as_array_mut) {
            ...
            .find(|message| message.get("role").and_then(Value::as_str) == Some("system"))
        {
            ...
            system["content"] = Value::String(format!("{current}{completion_instruction}"));
        }
        retry["options"]["num_predict"] = Value::from(super::OLLAMA_OUTPUT_TOKENS);
    } else {
        retry["max_output_tokens"] = Value::from(max_output_tokens);
    }
...
fn ollama_summary_payload(
    ...
    json!({
        ...
        "options": {
            ...
            "num_ctx": super::OLLAMA_CONTEXT_TOKENS
        }
    })
}
...
async fn post_summary_payload(
    ...
    let response = loop {
        let retry_client = (retry_count > 0).then(super::build_http_client);
        let client = retry_client
            .as_ref()
            .unwrap_or_else(|| super::http_client());
        let mut request_builder = client
            .post(endpoint)
            .header("Content-Type", "application/json")
            .json(payload);
        if provider == "ollama" {
            request_builder = request_builder.timeout(Duration::from_secs(120));
        } else {
    ...
    if provider == "ollama" && response.get("done_reason").and_then(Value::as_str) != Some("length")
    {
        super::validate_ollama_response(&response)?;
    }
    Ok(response)
}
...
pub async fn generate_topic_summary(request: SummaryRequest) -> Result<TopicSummaryInput, String> {
    ...
    let web_search = request.web_search && request.provider != "ollama";
    let shape = summary_shape(&request.kind, &request.transcript);
    let web_instruction = if web_search {
        if is_lecture {
    ...
    let base_url = super::text_provider_base_url(
        &request.provider,
    ...
    let max_output_tokens = if request.provider == "ollama" {
        super::OLLAMA_OUTPUT_TOKENS as i32
    } else if is_lecture {
        18_000
    } else {
        5_500
    };
    let mut payload = if request.provider == "ollama" {
        super::validate_ollama_input(&[&instructions, &input, &schema.to_string()])?;
    ...
    let api_key = if request.provider == "ollama" {
        ...
        let slot = if request.openai_key_slot.trim().is_empty() {
        ...
        super::read_provider_api_key(slot)?
    } else {
        super::read_provider_api_key(&request.provider)?
    };
    let endpoint = if request.provider == "ollama" {
    ...
    let first_parse = first_text
        ...
        .and_then(|text| {
            if request.provider == "ollama" {
                super::validate_ollama_response(&response_value)?;
            }
            parse_generated_summary(text)
        });
    ...
    if request.provider == "ollama" {
        generated.web_insights.clear();
    }
    let mut sources = if request.provider == "ollama" {
        Vec::new()
    } else {
        collect_sources(&response_value)
    };
    if !sources.is_empty() {
    ...
    Ok(TopicSummaryInput {
        ...
        web_enriched: web_search && !sources.is_empty(),
        sources,
...
mod tests {
    ...
    #[test]
    fn builds_native_ollama_summary_payload() {
        let payload = ollama_summary_payload(
            "qwen2.5:7b",
            "Return JSON.",
            "课堂内容",
            &json!({"type": "object"}),
        ...
        assert_eq!(
            ...
            super::super::OLLAMA_CONTEXT_TOKENS
        );
    ...
    #[test]
    fn extracts_native_ollama_message_content() {
        let response = json!({
            "message": {"role": "assistant", "content": "{\"title\":\"本地总结\"}"}
        });
        assert_eq!(output_text(&response), Some("{\"title\":\"本地总结\"}"));
    }

    #[test]
    fn local_summary_retry_stays_within_its_context_budget() {
        let payload = ollama_summary_payload(
            "qwen2.5:7b",
            "Return JSON.",
            "课堂内容",
            &json!({"type": "object"}),
            super::super::OLLAMA_OUTPUT_TOKENS as i32,
        );
        let retry = compact_summary_retry_payload(&payload, true);
        assert_eq!(
            retry["options"]["num_predict"],
            super::super::OLLAMA_OUTPUT_TOKENS
        );
        assert_eq!(
            retry["options"]["num_ctx"],
            super::super::OLLAMA_CONTEXT_TOKENS
        );
        assert!(retry.get("max_output_tokens").is_none());
        assert!(response_is_incomplete(
